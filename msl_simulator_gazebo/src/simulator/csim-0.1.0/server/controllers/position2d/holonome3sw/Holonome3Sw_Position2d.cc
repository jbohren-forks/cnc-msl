...
#include "simulator/csim-0.1.0/server/controllers/position2d/holonome3sw/Holonome3Sw_Position2d.hh"
...
void Holonome3Sw_Position2d::LoadChild(/*XMLConfigNode *node*/)
{
  ...
  this->userVelocities = false; // node->GetBool("uservelocities", false, 0);
  this->xya = Vector3(); // node->GetVector3("velocities", Vector3());
...
//    gzthrow("Holonome3Sw_Position2d controller requires a <wheels> node");
  ...
  // Get default params
  float distdef = 0;//node->GetFloat("distance", 0.0, 0);
  float radiusdef = 0;//node->GetFloat("radius", 0.0, 0);
  float maxtorquedef = 0;//node->GetFloat("torque", 0.0, 0);
  float betadef = 0;//node->GetFloat("beta", 0.0, 0);
  float gammadef = 0;//node->GetFloat("gamma", 0.0, 0);
...
//      gzthrow("The controller couldn't get swedish wheels " + sw.str());
    ...
    this->ALPHA[i] = 1;//lnode->GetFloat("alpha", 1000000, 1);
    if (this->ALPHA[i] == 1000000)
      gzthrow("The controller couldn't get alpha param for " + sw.str());

    this->DIST[i] = distdef; //lnode->GetFloat("distance", distdef, 0);
    if (!this->DIST[i])
      gzthrow("The controller could't get distance param for " + sw.str());

    this->BETA[i] = betadef; //lnode->GetFloat("beta", betadef, 0);

    this->GAMMA[i] = gammadef;//lnode->GetFloat("gamma", gammadef, 0);

    this->RADIUS[i] = radiusdef;//lnode->GetFloat("radius", radiusdef, 0);
    if (!this->RADIUS[i])
      gzthrow("The controller could't get radius param for " + sw.str());

    this->MAXTORQUE[i] = maxtorquedef; //lnode->GetFloat("torque", maxtorquedef, 0);
    if (!this->MAXTORQUE[i])
...
void Holonome3Sw_Position2d::GetPositionCmd(){
   ...
   // CMD_Vel vel;
    ...
    int lt = 100; //DB_get_from( this->selfID, this->selfID, CMD_VEL, (void *)&vel );

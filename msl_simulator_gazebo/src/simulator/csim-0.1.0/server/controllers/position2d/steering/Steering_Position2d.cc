...
void Steering_Position2d::LoadChild(XMLConfigNode *node)
{
  ...
  //general
  defaultTorque = 1000;//node->GetFloat("torque", 1000, 0);

  // Steering controller
  defaultSteerTorque = 100; //node->GetFloat("steerTorque", 100, 0);
  defaultKp = 0; //node->GetTupleDouble("steerPD", 0, 10.0);
...
// TODO XML STUFF
  ...
  while (childNode)
  {
    ...
    torque = defaultTorque; //childNode->GetFloat("torque", defaultTorque, 0);
    steerTorque = defaultSteerTorque; //node->GetFloat("steerTorque", defaultSteerTorque, 0);
    steerKp = 0;//node->GetTupleDouble("steerPD", 0, defaultKp);
    steerKd = 1;//node->GetTupleDouble("steerPD", 1, defaultKd);
    maxAngle = defaultMaxAngle;//node->GetFloat("steerMaxAngle", defaultMaxAngle, 0);

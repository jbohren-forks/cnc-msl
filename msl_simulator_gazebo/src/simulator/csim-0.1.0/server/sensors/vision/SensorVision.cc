...
#include "simulator/csim-0.1.0/server/sensors/vision/SensorVision.hh"


// CAMBADA include
#include "simulator/libs/rtdb/rtdb_sim.h"
#include "simulator/libs/rtdb/rtdb_user.h"
#include "simulator/libs/pman/pman.h"
#include "simulator/libs/pman/pmandefs.h"
#include "simulator/csim-0.1.0/agentWorld/WorldStateDefs.h"
#include "simulator/csim-0.1.0/geom/Vec.h"
//#include "Integrator.h"

#include "simulator/csim-0.1.0/geom/geometry.h"
...
static bool comparePosByAngle(cambada::geom::Vec first, cambada::geom::Vec second ){
...
{
  this->ballModelName = std::string(); //node->GetString("ballModel", std::string(), 1);
  this->ballRadius    =  0.11; //node->GetFloat("ballRadius", 0.11, 0);
  this->maxWhitePoints= 500; //node->GetInt("maxWhitePoints", 500, 0);
  this->seeDistance   = 5; //node->GetFloat("seeDistance", 5, 0);
  this->height        = 0.8; //node->GetFloat("height", 0.8, 0);
  this->cycleDelay    = 2; //node->GetInt("delay", 2, 0);
  this->radialSensors = 20; //node->GetInt("sensors", 20, 0);
  this->radialPasses  = 2; //node->GetInt("passes", 2, 0);

  this->noisyWhite    = false; // node->GetBool("noisyWhite",     false, 0);
  this->noisyBall     = false; //node->GetBool("noisyBall",      false, 0);
  this->noisyObstacles= false; //node->GetBool("noisyObstacles", false, 0);


  this->PMANConfigFile = std::string(); //node->GetFilename("PMANConf", std::string(), 0);
  //DONT NEED
